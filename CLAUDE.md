...
- **Use `.venv/`, never system Python**: Reuse the project's `.venv/` if it exists with the right Python version (match the notebook kernel for `.ipynb` projects); otherwise create one with python -m venv .venv, falling back to python3 -m venv .venv if python is unavailable. Run all commands through it (`.venv/bin/python`, `.venv/bin/pip`). Never use `python3` or `pip3`. Add `.venv/` to `.gitignore`.
